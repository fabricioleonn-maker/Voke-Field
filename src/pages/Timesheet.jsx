...
import { 
  Clock, ArrowLeft, FileText, Download, Plus, 
  Calendar, CheckCircle, XCircle, TrendingUp, CalendarDays
} from 'lucide-react';
    ...
    <div style={{ paddingBottom: '2rem' }}>
      ...
      <div className="card" style={{ marginBottom: '1rem', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
        <div>
          <h1 className="text-h2" style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', color: '#1e3a8a' }}>
            <Clock size={24} /> Folha de Ponto
          </h1>
          <p className="text-muted">Gerenciamento mensal de registros de ponto</p>
        </div>
        <div style={{ display: 'flex', gap: '0.5rem' }}>
          <button className="btn btn-neutral"><ArrowLeft size={16}/> Voltar</button>
          <button className="btn" style={{ backgroundColor: '#10b981', color: 'white' }}><Calendar size={16}/> Gerar Folha Automática</button>
          ...
          <button className="btn btn-primary"><Plus size={16}/> Novo Registro</button>
        </div>
      </div>
      ...
      <div className="card" style={{ marginBottom: '1.5rem', backgroundColor: '#eff6ff', border: '1px solid #bfdbfe' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          ...
          <div style={{ alignSelf: 'flex-start' }}>
            <label className="text-sm font-medium" style={{ display: 'block', marginBottom: '0.5rem', color: '#3b82f6' }}>Colaborador</label>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: '1rem', marginBottom: '1.5rem' }}>
        <div className="card" style={{ backgroundColor: '#3b82f6', color: 'white', padding: '1.5rem', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          <div>
            <div className="text-sm font-medium" style={{ opacity: 0.9, marginBottom: '0.5rem' }}>Total de Registros</div>
            <div className="text-h1">0</div>
          </div>
          <CalendarDays size={40} style={{ opacity: 0.8 }} />
        </div>
        <div className="card" style={{ backgroundColor: '#10b981', color: 'white', padding: '1.5rem', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          <div>
            <div className="text-sm font-medium" style={{ opacity: 0.9, marginBottom: '0.5rem' }}>Presenças</div>
            <div className="text-h1">0</div>
          </div>
        ...
        <div className="card" style={{ backgroundColor: '#ef4444', color: 'white', padding: '1.5rem', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          <div>
            <div className="text-sm font-medium" style={{ opacity: 0.9, marginBottom: '0.5rem' }}>Faltas</div>
            <div className="text-h1">0</div>
          </div>
        ...
        <div className="card" style={{ backgroundColor: '#8b5cf6', color: 'white', padding: '1.5rem', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          <div>
            <div className="text-sm font-medium" style={{ opacity: 0.9, marginBottom: '0.5rem' }}>Horas Extras</div>
            <div className="text-h1">0.0h</div>
          </div>
      ...
      <div className="card">
        <table className="data-table">
          <thead>
            <tr>
              <th className="text-xs text-muted" style={{ fontWeight: 600 }}>DATA</th>
              <th className="text-xs text-muted" style={{ fontWeight: 600 }}>COLABORADOR</th>
              <th className="text-xs text-muted" style={{ fontWeight: 600 }}>ENTRADA</th>
              <th className="text-xs text-muted" style={{ fontWeight: 600 }}>SAÍDA</th>
              <th className="text-xs text-muted" style={{ fontWeight: 600 }}>H. EXTRAS</th>
              ...
              <tr>
                <td colSpan="7">
                  <div style={{ padding: '4rem 0', textAlign: 'center', color: '#94a3b8' }}>
                    <CalendarDays size={48} style={{ opacity: 0.3, margin: '0 auto 1rem auto' }} />
                    <div className="text-lg font-medium" style={{ color: '#64748b', marginBottom: '0.25rem' }}>Nenhum registro encontrado</div>
                    <div className="text-sm">Adicione registros de ponto para o mês selecionado</div>
                  </div>
                </td>
              </tr>
            )}
